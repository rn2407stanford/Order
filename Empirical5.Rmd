...
p1=ggplot() + 
  ...
  theme_classic() +scale_color_manual(values = legend_colors)+ylab("Indices")+xlab("Item Number") + scale_x_continuous(limits = c(0, length(resp1)) , breaks = seq(1, length(resp1), by = 1), labels = paste0("Item", paste0(seq(1,length(resp1),1)) ), guide = guide_axis(angle = 45)) + geom_vline(xintercept = seq(from=1, to=length(resp1)), col="lightgray") + ggtitle("mpsycho_ceaq.Rdata") + ylim(-1.4,1.5)
...
ggplot() + 
  ...
  geom_line(data=df1b, mapping=aes(x=1:(length(ord)-1), y=df1b[,1], col="Index5"  ))+
  geom_line(data=d09, mapping=aes(x=1:(length(ord)-1), y=d09[,1] , col="Index6") ) +
  theme_classic() +scale_color_manual(values = legend_colors)+ylab("Indices")+xlab("Item Number")+ scale_x_continuous(limits = c(0, length(resp1)), breaks = seq(0, length(resp1), by = 1))+ geom_vline(xintercept = seq(from=1, to=length(resp1)),  col="lightgray")+ ggtitle("mpsycho_ceaq.Rdata")
ggsave("Empirical5.jpg", p1, height=5)
p1
```

```{r}
empirical5=list(Index1=data.frame(z1), Index2=data.frame(s4[,7]), Index3=d08, Index4=df1., Index5=df1b, Index6=d09)
