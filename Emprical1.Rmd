...
plot(1:ncol(resp1), s4[,7], xlab='Item Number',ylab='order', main="Index 2")
data.frame(s4[,7])
s4
```
...
plot(1:(length(ord)-1), df1.[,1], main="Index 4", xlab="Item Number")
df1.
```
...
ggplot() + 
  ...
  theme_classic() +scale_color_manual(values = legend_colors)+ylab("Indices")+xlab("Item Number") + scale_x_continuous(limits = c(0, length(resp1)), breaks = seq(0, length(resp1), by = 1)) + geom_vline(xintercept = seq(from=1, to=length(resp1)), col="lightgray") + ggtitle("mpsycho_learnemo.Rdata") + ylim(-0.5,1)
...
ggplot() + 
  ...
  geom_line(data=df1b, mapping=aes(x=1:(length(ord)-1), y=df1b[,1], col="Index5"  ))+
  geom_line(data=d09, mapping=aes(x=1:(length(ord)-1), y=d09[,1] , col="Index6") ) +
  theme_classic() +scale_color_manual(values = legend_colors)+ylab("Indices")+xlab("Item Number")+ scale_x_continuous(limits = c(0, length(resp1)), breaks = seq(0, length(resp1), by = 1))+ geom_vline(xintercept = seq(from=1, to=length(resp1)),  col="lightgray")+ ggtitle("mpsycho_learnemo.Rdata") 
```

```{r}
empirical1=list(Index1=data.frame(z1), Index2=data.frame(s4[,7]), Index3=d08, Index4=df1., Index5=df1b, Index6=d09)
